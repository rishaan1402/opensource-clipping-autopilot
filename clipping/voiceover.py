...
def get_commentary_prompt(
    transcript_snippet: str, style: str, language: str, length: str, strict_commentary: bool = False
) -> str:
    ...
    if strict_commentary:
        ...
        if language == "en":
            strict_instruction = (
                "\nADDITIONAL RULE (MANDATORY — this source has no license or explicit "
                "permission, so this video must stand as original commentary, not a re-narration):\n"
                "- Provide a critique, analysis, or perspective that is SUBSTANTIALLY new — "
                "not a paraphrase of what's said in the clip.\n"
                "- Reference the clip's content only enough to give your commentary context; "
                "the core of this video must be your opinion, not the original content.\n"
                "- Do not simply re-describe what happens in the clip, word for word."
            )
...
def generate_commentary_script(
    transcript_snippet: str, cfg, style="analysis", language="id", length="short", strict_commentary=False
) -> str:
