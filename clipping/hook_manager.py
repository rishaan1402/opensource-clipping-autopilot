...
def download_custom_hook(cfg) -> str | None:
    """
    Downloads or copies a single custom hook file from the --hook-source argument.
    Returns: absolute path to the local file, or None if failed/not provided.
    """
    ...
    if source.startswith("http"):
        ...
        try:
            ...
            if os.path.exists(local_path):
                print(f"   ✅ Custom hook downloaded successfully to {local_path}")
                return local_path
            else:
