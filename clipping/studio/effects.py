...
def prepare_glitch_video(ratio, cfg, video_encoder, source_h=1080, custom_dims=None):
    ...
    if use_downloaded and os.path.exists(glitch_raw):
        ...
        # Audio MUST NOT be dead silence here: anullsrc produces true digital
        # silence, which — right after a 3s hook, often with no BGM yet —
        # reads to a viewer as the clip going dead/broken for a full second,
        # not as an intentional transition. A short white-noise/static burst
        # (anoisesrc) matches what a "TV glitch" transition should actually
        # sound like, and a snappier 0.4s duration keeps it feeling like a
        ...
        cmd = (
            [
                ...
                "-f", "lavfi",
                "-i", f"anoisesrc=d={duration}:c=white:r=48000:a=0.25",
                "-t", str(duration),
